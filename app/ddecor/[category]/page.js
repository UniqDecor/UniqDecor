...
export default async function CategoryPage({ params }) {
  ...
  const productSchema = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": `D'Decor ${data.categoryName} at Uniq Decor Udaipur`,
    "image": data.mainImage,
    "description": data.metaDesc,
    "brand": {
      "@type": "Brand",
  ...
  const getWhatsAppLink = (catName) => {
    const encodedMsg = encodeURIComponent(`Hi Uniq Decor! 👋 I'm interested in D'Decor ${catName} at your Udaipur showroom. Please share the fabric catalog and estimated pricing.`);
    ...
    <div className="theme-ddecor bg-[#FAF9F6] text-[#2D2A26] min-h-screen">
      ...
      <header className="relative min-h-[50vh] md:min-h-[60vh] flex items-center justify-center overflow-hidden bg-[#2D2A26] pt-32 pb-16">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <span className="inline-block text-xs uppercase tracking-[0.3em] text-[#C9A227] font-bold mb-4">
            D'Decor Udaipur &bull; {data.categoryName}
          </span>
      ...
      <section className="py-16 px-4 md:px-12 max-w-7xl mx-auto border-b border-[#8B4513]/10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 flex flex-col gap-6">
            <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#8B4513]">Local Showroom Collections</span>
            <h2 className="font-serif text-3xl font-bold text-[#2D2A26]">Premium Fabric Curations in Udaipur</h2>
          ...
          <div className="lg:col-span-5 bg-[#F5F0E8] border border-[#8B4513]/10 p-8 rounded-2xl flex flex-col gap-6">
            <h3 className="font-serif text-lg font-bold text-[#2D2A26] border-b border-[#8B4513]/10 pb-3">Quality Highlights</h3>
      ...
      <section className="py-20 px-4 md:px-12 max-w-7xl mx-auto border-b border-[#8B4513]/10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#8B4513]">Explore Weaves</span>
          <h2 className="font-serif text-3xl md:text-4xl font-bold mt-2 text-[#2D2A26]">Atelier Product Range</h2>
          <p className="text-[#6B6560] text-xs md:text-sm mt-2">
            A look inside D'Decor's flagship varieties and texture finishes available on order at our Udaipur showroom.
          </p>
        </div>
          ...
          {data.items.map((item, idx) => (
            <div key={idx} className="bg-white border border-[#8B4513]/8 rounded-[20px] overflow-hidden shadow-sm hover:translate-y-[-6px] hover:shadow-lg transition-all duration-400 group">
              <div className="h-56 overflow-hidden relative bg-stone-100">
                <Image
                  src={item.img}
                  alt={`${item.title} D'Decor ${data.categoryName} store Udaipur`}
                  fill
      ...
      {/* SCHEMA COMPLIANT FAQ ACCORDION */}
      <section className="py-20 px-4 md:px-12 max-w-4xl mx-auto border-b border-[#8B4513]/10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="text-xs uppercase tracking-[0.2em] font-bold text-[#8B4513]">Answering Questions</span>
          <h2 className="font-serif text-3xl font-bold mt-2 text-[#2D2A26]">Frequently Asked Questions</h2>
          <p className="text-[#6B6560] text-xs mt-2">Find details about fabric catalog selection, custom tailoring, and showroom consults in Udaipur.</p>
        </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <p className="text-[#6B6560] text-xs md:text-sm leading-relaxed">
              Step inside our physical store in Udaipur to experience the weight, texture, and actual shades of our D'Decor drapery. Our consultants offer loose fabric supply or complete tailored installations.
            </p>
      ...
      {/* INTERNAL LINKING / D'DECOR CATEGORIES BAR */}
      <section className="py-16 bg-[#F5F0E8] border-t border-[#8B4513]/10">
        <div className="max-w-7xl mx-auto px-4 md:px-12 text-center">
          <span className="text-xs uppercase tracking-widest text-[#8B4513] font-bold block mb-6">Explore Other Collections</span>
          <div className="flex flex-wrap justify-center items-center gap-4">
            {Object.entries(CATEGORIES_DATA).map(([key, item]) => {
...
// Inline styling custom override to hide details summary arrows in some browsers
const styleTag = (
