        ...
        {data && !loading && (
          <div className="bg-white border border-[#8B4513]/10 p-6 rounded-2xl shadow-sm mb-8 flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto shrink-0 justify-end">
              <button
                onClick={() => {
                  let tableText = "Route Path\tTarget Focus Keyword\n";
                  data.pages.forEach(p => {
                    tableText += `${p.path}\t${p.keyword || "None"}\n`;
                  });
                ...
                className="inline-flex items-center gap-2 px-3.5 py-2.5 border border-[#8B4513]/15 hover:border-[#C5A059]/40 hover:bg-[#FAF9F6] text-[#8B4513] font-bold text-xs uppercase tracking-wider rounded-lg transition-all cursor-pointer shadow-sm active:scale-95 shrink-0 bg-white"
                title="Copy all route keywords in tab-separated tabular format"
              >
                {copiedAllKeywords ? (
                  <>
          ...
          <div className="flex items-start gap-4 grow">
            ...
            <div className="grow">
              ...
              <div className="mt-2.5">
                ...
                <span className="text-base font-serif font-black text-[#8B4513] tracking-wide mt-0.5 block flex items-center gap-2">
                  <Tag className="w-4 h-4 text-[#C5A059] shrink-0" />
                  <span>"{page.keyword || "None"}"</span>
                  <span className="text-[9px] font-mono font-bold bg-[#FAF9F6] border border-[#8B4513]/10 text-[#6B6560] px-2 py-0.5 rounded-full uppercase">
                    {page.keywordType}
                  </span>
                  {page.keyword && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        navigator.clipboard.writeText(page.keyword);
                        setCopiedKeywords(prev => ({ ...prev, [page.path]: true }));
                        setTimeout(() => {
                          setCopiedKeywords(prev => ({ ...prev, [page.path]: false }));
                        }, 2000);
                      }}
                      className="p-1 hover:bg-stone-100 text-[#6B6560] hover:text-[#8B4513] rounded-md transition-all cursor-pointer flex items-center justify-center border border-stone-200/60 bg-stone-50/50"
                      title="Copy focus keyword to clipboard"
                    >
                      {copiedKeywords[page.path] ? (
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-600" />
                      ) : (
                        <Copy className="w-3.5 h-3.5 text-[#C5A059]" />
                      )}
              ...
              <div className="mt-3.5 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-stone-100 pt-3.5">
                <div className="bg-stone-50/50 border border-stone-200/50 p-2.5 rounded-xl">
                  <span className="text-[9px] uppercase font-bold text-[#6B6560] tracking-wider block">Meta Title ({page.title?.value?.length || 0} chars)</span>
                  <span className="text-xs font-semibold text-stone-850 mt-0.5 block font-sans truncate" title={page.title?.value}>{page.title?.value || "Missing Meta Title"}</span>
                </div>
                <div className="bg-stone-50/50 border border-stone-200/50 p-2.5 rounded-xl">
                  <span className="text-[9px] uppercase font-bold text-[#6B6560] tracking-wider block">Meta Description ({page.description?.value?.length || 0} chars)</span>
                  <span className="text-xs text-stone-600 mt-0.5 block font-sans line-clamp-2" title={page.description?.value}>{page.description?.value || "Missing Meta Description"}</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-4 text-[10px] text-[#6B6560] font-medium bg-[#FAF9F6] border border-[#8B4513]/5 p-2 px-3 rounded-lg w-fit">
                ...
                <span className="flex items-center gap-1"><TrendingUp className="w-3.5 h-3.5 text-indigo-500" /> Rank #<strong>{page.ranking.position}</strong></span>
